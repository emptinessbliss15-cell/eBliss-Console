...
import { Menu, X, LogIn, LogOut, ChevronDown, LifeBuoy } from 'lucide-react'
...
export default function App() {
  ...
  function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    setAuthenticated(true); setPassword(''); setLoginError(''); setLoginOpen(false); setMenuOpen(false)
  }
  ...
  return <div className={`console theme-${theme}`}>
    ...
    {menuOpen && <aside className="app-menu" aria-label="Apps">
      <div className="menu-title">Apps</div>
      {authenticated && <button className={`app-link ${activeApp === 'Supportable' ? 'active' : ''}`} title="Supportable" aria-label="Supportable" onClick={() => { setActiveApp('Supportable'); setMenuOpen(false) }}><LifeBuoy size={24} /></button>}
