...
export default function App() {
  ...
  return <div className={`console theme-${theme}`}>
    <header className="console-header">
      ...
      {authenticated && <div className="header-actions"><span className="user-email">{userEmail}</span><label className="theme-picker"><span className="sr-only">Theme</span><select value={theme} onChange={(event) => setTheme(event.target.value as ThemeName)}>{Object.entries(themes).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><ChevronDown size={15} /></label></div>}
    </header>
    {menuOpen && <aside className="app-menu">
      ...
      {!authenticated ? <button className="app-link menu-action" onClick={() => { setLoginError(''); setLoginOpen(true) }}><LogIn size={17} /> Login</button> : <button className="app-link menu-action" onClick={handleLogout}><LogOut size={17} /> Logout</button>}
